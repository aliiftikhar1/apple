...
const FilterableTable = ({ data, fetchData,userRole, userBranch }) => {
  ...
  const fetchBranches = async () => {
    try {
     const response = await fetch('https://apple-immi-solution.vercel.app/api/branches');
  ...
  const handleAddOrUpdateItem = async () => {
    ...
    const url = newItem.id ? `https://apple-immi-solution.vercel.app/api/admin/${newItem.id}` : 'https://apple-immi-solution.vercel.app/api/admin';
    try {
  ...
  const handleDeleteItem = async (id) => {
    setIsLoading(true);
    try {
      const response = await fetch(`https://apple-immi-solution.vercel.app/api/admin/${id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      });
      const result = await response.json();
