...
const FilterableTable = ({ data, userRole, userBranch, userId, filetypedata, fetchData }) => {
  ...
  useEffect(() => {
    ...
    if (userRole === 'manager') {
      fetch(`https://apple-immi-solution.vercel.app/api/branchuser/${userId}`)
  ...
  const handleAddOrUpdateItem = async (item) => {
    ...
    const url = item.id ? `https://apple-immi-solution.vercel.app/api/user/${item.id}` : 'https://apple-immi-solution.vercel.app/api/user';
  ...
  const handleDeleteItem = async (id) => {
    setIsLoading(true);
    try {
      const response = await fetch(`https://apple-immi-solution.vercel.app/api/user/${id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
