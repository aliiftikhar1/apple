...
const UserDetailsCard = ({ item, handleEditItem, handleDeleteItem }) => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        const response = await fetch('https://apple-immi-solution.vercel.app/api/getimages', {
          method: 'POST',
          headers: {
      ...
      <html>
        ...
        <body>
          <div class="print-content">
            <div class="header">
              <img src="http://admin.applelegal.co/uploads/${item.imgurl}" alt="User Profile" />
              <div class="details">
  ...
  return (
    <div className="bg-white rounded-xl shadow-md hover:shadow-lg overflow-hidden relative p-4">
      <div className="flex justify-center items-center p-4 border-b">
        <div className='flex flex-col justify-center items-center'>
          <img className='w-[150px] h-[150px] rounded-full mr-4 object-cover' src={`http://admin.applelegal.co/uploads/${item.imgurl}`} alt="User Profile" />
        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-3xl w-full relative">
            ...
            <div id={`print-content-${item.id}`} className="hidden print:block">
              <div class="header">
                <img src={`http://admin.applelegal.co/uploads/${item.imgurl}`} alt="User Profile" style={{ objectFit: 'cover' }} />
                <h2>${item.Full_Name}</h2>
              </div>
              <div class="grid">
                <div class="column">
    ...
    {uploadedFiles.map((file, index) => (
      <div key={index} className="flex flex-col items-center border rounded p-2">
        <img src={`http://admin.applelegal.co/uploads/${file.imgurl}`} alt={file.image_name} className="w-full h-48 object-cover mb-2" />
        <p className="text-sm">{file.image_name}</p>
        <div className="flex mt-2">
          <a href={`http://admin.applelegal.co/uploads/${file.imgurl}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline px-2">
            <FaEye />
          </a>
          <button onClick={() => handleDownload(`http://admin.applelegal.co/uploads/${file.imgurl}`, file.imgurl)} className="text-green-500 hover:underline px-2">
      <FaDownload />
          </button>
        </div>
      </div>
