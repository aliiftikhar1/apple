...
const FilterType = () => {
  ...
  const fetchFiletypes = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('https://apple-immi-solution.vercel.app/api/filetype');
  ...
  const handleDeleteFiletype = async (id) => {
    setIsLoading(true);
    try {
      const response = await fetch(`https://apple-immi-solution.vercel.app/api/filetype/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
  ...
  const handleAddOrUpdateFiletype = async () => {
    ...
    const endpoint = newFiletype.id ? `https://apple-immi-solution.vercel.app/api/filetype/${newFiletype.id}` : 'https://apple-immi-solution.vercel.app/api/filetype';
